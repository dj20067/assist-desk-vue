...
const CustomerServiceWorkspace: React.FC<CustomerServiceWorkspaceProps> = ({
  ...
  // 生成信息面板配置
  const generateInfoPanels = (): InfoPanelItem[] => [
    {
      key: 'customer',
      header: (
      ...
      content: (
        <div className="service-history">
          <List 
            size="small" 
            dataSource={[
              { date: '2024-01-15', summary: 'RPA流程配置问题咨询', status: '已解决' },
              { date: '2024-01-10', summary: '系统登录异常', status: '已解决' }
            ]} 
            renderItem={item => (
  ...
  return <Layout className="customer-service-workspace">
      ...
      {/* 右侧信息区 */}
      <InfoSidebar 
        panels={generateInfoPanels()}
        defaultActiveKeys={['customer', 'app', 'history', 'moreInfo']}
        width="30%"
      />
